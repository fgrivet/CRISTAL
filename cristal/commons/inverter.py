...
class Inverter(BaseCommons, Generic[ArrayLike, DTypeLike]):
    ...
    def invert(self, X: ArrayLike, eps: float | None = None) -> ArrayLike:
        ...
        eps : float | None, optional
            The regularization to add to the matrix before inverting it: :math:`X_{inv} = (X + \\epsilon I)^{-1}`.
            If None, defaults to :attr:`eps`, by default None.
    ...
    def __call__(self, X: ArrayLike, eps: float | None = None) -> ArrayLike:
        ...
        X : ArrayLike
            The matrix to invert.
        eps : float | None, optional
            The regularization to add to the matrix before inverting it: :math:`X_{inv} = (X + \\eps I)^{-1}`.
            If None, defaults to :attr:`eps`, by default None.
