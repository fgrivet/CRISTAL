"""
This module implements the :class:`DyCF` (Dynamical Christoffel Function) and :class:`DyCG` (Dynamical Christoffel Growth)
anomaly detection algorithms. It also includes a :class:`BaggingDyCF` class for ensemble learning with DyCF models.
"""
...
from ..type_checking import check_all_int, check_types, positive_integer
...
class DyCF(BaseDetector):
    ...
    def __init__(
        ...
        n: int
            The degree of polynomials, usually set between 2 and 8.
        regularizer_opt: IMPLEMENTED_REGULARIZERS, optional, by default IMPLEMENTED_REGULARIZERS.VU_C
            The regularization method to use to divide the scores.
            See :attr:`~cristal.regularizers.IMPLEMENTED_REGULARIZERS` for more details.
        C: float | int, optional, by default 1
            The constant factor used in the :const:`"vu_C"` or :const:`"constant"` regularizer.
        polynomial_family_opt: IMPLEMENTED_POLYNOMIALS, optional, by default IMPLEMENTED_POLYNOMIALS.MONOMIALS
            The polynomial basis to use for the moments matrix.
            See :attr:`~cristal.polynomials.IMPLEMENTED_POLYNOMIALS` for more details.
        inverter_opt: IMPLEMENTED_INVERTERS, optional, by default IMPLEMENTED_INVERTERS.FPD
            The inversion option to use for the moments matrix.
            See :attr:`~cristal.inverters.IMPLEMENTED_INVERTERS` for more details.
        incrementer_opt: IMPLEMENTED_INCREMENTERS, optional, by default IMPLEMENTED_INCREMENTERS.INVERSE
    ...
    def score_samples(self, x):
        self.assert_shape_fitted(x)
        return self.moments_matrix.score_samples(x.reshape(-1, self.d)) / self.regularizer_value
    ...
    def is_fitted(self):
        return self.moments_matrix.is_fitted() and self.d > 0 and self.regularizer_value is not None
...
class BaggingDyCF(DyCF):
    ...
    def __init__(self, n_values: int | list, n_estimators: int = 10, **dycf_kwargs):
        ...
        self.regularizer_opt = self.models[0].regularizer_opt
        self.C = self.models[0].C
        # Initialize the regularizer_value to None, it will be set during the fit method
        self.regularizer_value = None
